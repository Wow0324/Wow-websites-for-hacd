...
const QuoteFee = () => {
    ...
    const handleResetFee = async () => {
        ...
        if(txhash == "" || fee == "" || password == ""){
            ...
            try {
                const response = await axios.get(`http://localhost:8001/api/diamond/quotefee?txhash=${txhash}&fee=${fee}&password=${password}`)
    ...
    return (
        <div className="container mx-auto mt-10 bg-gradient-to-r from-[#bb71ff] to-[#924dea] text-white rounded">
            <h1 className="text-[14px] text-lg m-1 p-5">Fee competition:</h1>
