...
const TABLE_HEAD = ["#", "Diamond Name", "Reward Address", "Tx Hash", "Fee Offer", "Fee Address", "Status"];
...
const BiddingTable: React.FC<BidsProps> = ({ period, number, data }) => {
    ...
    return (
        ...
        <div className="container mt-10 bg-gradient-to-r from-[#bb71ff] to-[#924dea] mx-auto text-white rounded">
            <h1 className="text-[14px] mb-2 p-5">
                ...
                <table className="table-auto w-full text-left border border-white">
                    ...
                    <tbody>
                        {data.map((item, index) => {
                            ...
                            return (
                                <tr key={index}>
                                    <td className="text-[12px] p-4">{item[0]}</td>
                                    <td className="text-[14px] p-2">{item[3]}</td>
                                    <td className="text-[12px] p-2">
                                        <a target="_blank" title={`${item[4]}`} href={`https://explorer.hacash.diamonds/address/${item[4]}`}>
                                            {item[4]}
                                        </a>
                                    </td>
                                    <td className="text-[12px] p-2">
                                        <div className="container flex flex-row">
                                            ...
                                            <div className="pr-2">
                                                <button className="text-[10px] underline" onClick={() => handleCopyToClipboard(item[1])}>Copy</button>
                                            </div>
                                        </div>


                                    </td>
                                    <td className="text-[12px] p-2">{item[5]}</td>
                                    <td className="text-[12px] p-2">
                                        <a target="_blank" title={`${item[2]}`} href={`https://explorer.hacash.diamonds/address/${item[2]}`}>
                                            {item[2]}
                                        </a>
                                    </td>
                                    <td className="text-[12px] p-4">
                                        {item[6] == 0 ? "ok" : ""}
                                    </td>
                                </tr>
                            )
