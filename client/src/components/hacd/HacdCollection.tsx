...
const HacdCollection = () => {
    ...
    return (
        <div className="container mx-auto my-7 sm:my-18">
            <h1 className="text-center text-white font-bold sm:text-5xl text-3xl">Pure HACD Collection</h1>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 my-5 sm:gap-10 sm:my-20 mx-5">
                {diamonds.map((item, index) => (
                    <img
                        key={index}
                        className="h-96 w-full rounded-lg object-cover object-center shadow-xl shadow-light/50 hover:scale-105"
                        src={item.image}
                        alt={`Image ${index + 1}`}
                    />
                ))}
